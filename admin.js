...
let products=readJSON('ba_products',[]);let orders=readJSON('ba_orders',[]);if(!Array.isArray(products)||!products.length){products=DEMO_PRODUCTS.map(p=>({...p}));localStorage.setItem('ba_products',JSON.stringify(products));}if(!Array.isArray(orders))orders=[];let editId=null;
...
function renderProducts(){let q=($('#productSearch').value||'').toLowerCase();$('#productsBody').innerHTML=products.filter(p=>String(p.name||'').toLowerCase().includes(q)).map(p=>`<tr><td>${p.image?`<img class="thumb" src="${esc(p.image)}">`:'—'}</td><td>${esc(p.name)}</td><td>${esc(p.cat)}</td><td>${Number(p.price).toFixed(2)} د</td><td>${p.stock}</td><td>${p.active===false?'مخفي':'ظاهر'}</td><td><div class="actions"><button class="edit" data-edit="${p.id}">تعديل</button><button class="hide" data-toggle="${p.id}">${p.active===false?'إظهار':'إخفاء'}</button><button class="delete" data-delete="${p.id}">حذف</button></div></td></tr>`).join('')||'<tr><td colspan="7">لا توجد منتجات</td></tr>'}
const labels={new:'جديد',contacted:'تم التواصل',confirmed:'مؤكد',shipped:'تم الشحن',cancelled:'ملغى'};
function renderOrders(){let q=($('#orderSearch').value||'').toLowerCase();$('#ordersBody').innerHTML=orders.filter(o=>String((o.name||'')+' '+(o.phone||'')+' '+(o.city||'')).toLowerCase().includes(q)).map(o=>`<tr><td>${esc(o.name)}</td><td><a href="tel:${esc(o.phone)}">${esc(o.phone)}</a></td><td>${esc(o.city)}</td><td>${Number(o.total).toFixed(2)} د</td><td><div class="order-items">${(o.items||[]).map(i=>`<div class="order-item"><div class="order-thumb">${i.image?`<img src="${esc(i.image)}" alt="${esc(i.name)}">`:'✦'}</div><div><b>${esc(i.name)}</b><small>${esc(i.cat||'')} — ${i.qty} × ${Number(i.price).toFixed(2)} د</small></div></div>`).join('')}</div></td><td><select class="status" data-status="${o.id}">${Object.entries(labels).map(([k,v])=>`<option value="${k}" ${o.status===k?'selected':''}>${v}</option>`).join('')}</select></td><td><div class="order-actions"><button class="details-btn" data-details="${esc(o.id)}">التفاصيل</button><button class="pdf-btn" data-pdf="${esc(o.id)}">PDF</button></div></td></tr>`).join('')||'<tr><td colspan="7">لا توجد طلبات</td></tr>'}
...
function commit(data){if(editId){const i=products.findIndex(p=>p.id===editId);if(i>=0)products[i]=data;else products.unshift(data)}else products.unshift(data);save();reset();$('#formMsg').textContent='تم الحفظ بنجاح.';renderProducts();stats()}
